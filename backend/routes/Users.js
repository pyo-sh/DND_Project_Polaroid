...
users.get("/finduser", (req, res, next) => {
	// 유저 찾기
		...
		if (info !== undefined) {
			...
			User.findOne({
				...
				if (userInfo !== null) {
					...
					res.status(401).send(
						"no user exists in db with that username"
					);
				}
			})(req, res, next);
		}
...
users.post("/findpassword", (req, res) => {
	...
	User.findAll({
		...
		if (user === null) {
			...
			const mailOptions = {
				...
				text:
					`안녕하세요.${req.body.email}님 Polaroid입니다. 아이디를 찾기 위해 요청을 하셨군요.\n\n` +
					`고객님의 아이디는 ${array} 입니다.\n\n` +
